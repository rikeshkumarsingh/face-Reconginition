...
import util
from test import test  # Replace with the correct module name where `test` is defined
...
class App:
    ...
    def add_webcam(self, label):
        if 'cap' not in self.__dict__:
            self.cap = cv2.VideoCapture(0)  # Use a more common camera index (0)
    ...
    def login(self):
        ...
        #      model_dir='C:/Rikesh/H/python/face-attendance-system-master',
        #     device_id=0
        # )


        # if label == 1:
        lable = test(image=self.most_recent_capture_arr,

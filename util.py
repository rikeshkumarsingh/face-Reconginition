    ...
    for filename in os.listdir(db_dir):
        if filename.endswith('.pickle'):
            with open(os.path.join(db_dir, filename), 'rb') as file:
                ...
                # Ensure user_data is a dictionary and contains 'embeddings'
                if isinstance(user_data, dict) and 'embeddings' in user_data:
                    known_embedding = user_data['embeddings']
                    results = face_recognition.compare_faces([known_embedding], embeddings[0])

                    if results[0]:
                        return user_data  # Return the whole user data dictionary
